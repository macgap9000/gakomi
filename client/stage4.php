...
<?php
  ...
  if (isset($_GET['initialCityName']))
  {
    ...
    {
      // Weryfikacja poprawna. Przesłano parametr tekstowy.
      // Wszystkie parametry z tablicy $_GET zaakceptowane.
      // Pobierz je i przygotuj treść parametrów do przekierowania:
      echo $query = http_build_query($_GET);

      // Wykonaj faktyczne przekierowanie:
      header('Location: stage5.php'.'?'.$query);
      exit();
    }    
  }
...
<html lang="pl">
  <head>
    ...
    <title>Gakomi - Obliczenia (4)</title>
  ...
  <body>
    ...
    <div class="container">
        ...
        <div class="well">
          Etap 1 &raquo; Etap 2 &raquo; Etap 3 &raquo; <b>Etap 4</b>
        </div>

        <form action="" method="GET">
          <div class="panel panel-default">
              <div class="panel-heading">
                <h3 class="panel-title">Wybierz miasto startowe</h3>
              </div>
              <div class="panel-body" style="padding-top:32px; padding-bottom:32px;">
                ...
                <div class="form-group">
                  <label>Wybierz miasto początkowe, z którego komiwojażer rozpoczyna swoją podróż:</label>
                  <select class="form-control" name="initialCityName">
                  <?php
                    // Ustawienie iteratora na pierwszą wartość:
                    $i = 1;                    

                    // Drukowanie miast startowych do wyboru:
                    foreach($cities as $city)
                    {
                      ...
                      // Wydrukowanie na ekran:
                      echo '<option value="'.$cityName.'">'.$cityName.'</option>';

                      // Zwiększenie wartości iteratora:
                      $i++;
                    }
                  ?>
                  </select>
                  <p>Wskaż miasto początkowe z rozwijalnej listy.</p>
                </div>
